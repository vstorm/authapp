...
const (
	errorsPackage = protogen.GoImportPath("github.com/go-kratos/kratos/v2/errors") 	// todo
	fmtPackage    = protogen.GoImportPath("fmt")
)

// generateFile generates a _errors.pb.go file containing go-klion errors definitions.
func generateFile(gen *protogen.Plugin, file *protogen.File) *protogen.GeneratedFile {
	...
	g.P()
	g.QualifiedGoIdent(fmtPackage.Ident(""))		// 生成fmt
	generateFileContent(gen, file, g)
	return g
}

// generateFileContent generates the go-klion errors definitions, excluding the package statement.
func generateFileContent(gen *protogen.Plugin, file *protogen.File, g *protogen.GeneratedFile) {
	...
	g.P("// is compatible with the go-klion package it is being compiled against.")
...
func genErrorsReason(gen *protogen.Plugin, file *protogen.File, g *protogen.GeneratedFile, enum *protogen.Enum) bool {
	...
	if code < 900 {
		panic(fmt.Sprintf("Enum '%s' range must be greater than 900", string(enum.Desc.Name())))
	}
	var ew errorWrapper
	for _, v := range enum.Values {
		...
		if enumCode < 900 {
			panic(fmt.Sprintf("Enum '%s' range must be greater than 900", string(v.Desc.Name())))
		}
		if enumCode == 0 {
			continue
		}
